...
public class Graf {
    String fileName = "src/prednaskaLABEL.hrn";
    int pocetVrcholov = 1;
    int pocetHran = 0;
    ...
    public void cyklus() {
        ...
        // cyklus zacitana zaciatocnym vrcholom, ktory pouzivatel zadal
        ...
        if (!this.mnozinaE.isEmpty()) {
            ...
            for (Hrana h : this.zoznamHran) {
                ...
                if (h.getZaciatocnyVrchol() == prvyVrchol.getCisloVrcholu()) {
                    ...
                    // potom pridaj tento koncovy bod do  mnozinyE na kontrolu
        ...
        //tuto len aktualizujem mnozinuE, kedze som nemohol pridavat vrcholy do arraylistu cez ktory ide cyklus
        for (Vrchol pomocnyE : pomocnaMnozinaE) {
            if (!this.mnozinaE.contains(pomocnyE)) { this.mnozinaE.add(pomocnyE); }
        }
        ...
        if (this.vypisPostupu==1) {
            for (Vrchol v : this.mnozinaE) { v.vypis(); }
            System.out.println("----------------------------------------------------------------");
        }

        if (!this.mnozinaE.isEmpty()) { this.cyklus(); }
        else {
            this.spracujCestu();
            this.vypisCestu();
        }
    ...
    public void spracujCestu() {
        // cyklus funguje tak, ze sa pozeram na atribut predposledneho vrchola vo vrcholoch a takto ich vsetky prejdem
